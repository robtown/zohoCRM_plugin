...
// echo $options['zohoCRM_api_key'];
$ghjb_d = false;
if ( $options['zohoCRM_debug'] === 'true' ) {
	$ghjb_d = true;	
}
$ghjb_e = false;
if ( $options['zohoCRM_log_errors'] === 'true' ) {
	$ghjb_e = true;	
}
...
// 		$job_json = wp_remote_retrieve_body( wp_remote_get('https://api.greenhouse.io/v1/boards/' . $options['zohoCRM_url_token'] . '/embed/job?id=' . $job->id . '&questions=true'));
		
// 		$ghjb_jobs .= $job_json . ',';
// 	}
// 	$ghjb_jobs = '[' . $ghjb_jobs . ']';
// 	set_transient( 'ghjb_jobs', $ghjb_jobs, $options['zohoCRM_cache_expiry'] );
...
if ( $ghjb_e &&
	 $_POST['id'] ) {
	$job = wp_remote_retrieve_body( wp_remote_get('https://api.greenhouse.io/v1/boards/' . $options['zohoCRM_url_token'] . '/embed/job?id=' . $_POST['id'] . '&questions=true'));
...
$url = "https://" . $options['zohoCRM_api_key'] . ":@api.greenhouse.io/v1/applications/";
